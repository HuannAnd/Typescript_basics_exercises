...
export const FirstExercise6 = (candidatesNames: string[], candidatesVotes: number[], password: string) => {
  ...
  const candidates: {name: string ; votes: number}[] = []
  for (let index = 1; index <= 3; index++) {
    if (index === 1) {
      ...
      for (let index = 0; index < candidatesNames.length; index++) { candidates.push({ name: candidatesNames[index], votes: 0 }) }
    } else if (index === 2) {
      ...
      console.log(candidates.filter(x => x.votes === maxVote).length > 1
        ? 'SEGUNDO TURNO'
        : `O candidato com mais votos é ${verificationNameCandidate} com ${maxVote} votos.`
      )
    }
  }
}
...
export const FirstExercise9 = (x: number, y: number, z: number) => {
  ...
  console.log(x > y + z
    ? 'Infelizmente não é um triângulo(x > y + z)'
    : 'Até pode ser um triângulo(x < y + z)'
  )
}
...
export const FirstExercise14 = (nms1: number, nms2: number, nms3: number) => {
  ...
  const nmsArrayPos1 = conversor.filter(x => ![nmsArrayPos0, nmsArrayPos2].includes(x))

  if (nmsArrayPos1.length === 0) {
    const nmsArrayPos11 = Math.min(...conversor.filter(x => x === nmsArrayPos0 || x === nmsArrayPos2))
